...
export default function EditCustomer(props) {
    ...
    const editUser = () => {
        props.editUser(editedFirstName, editedSurName,
        ...
        let userEdit = (
            ...
            <div><br/>First Name: <br/><input type="text" placeholder={props.user.firstName} minLength = "10" className={ styles.enterUser } 
                onChange={ (event) => setFirstName(event.target.value) }></input></div>
            
            <div>Surname: <br/><input type="text" placeholder={props.user.surName} minLength = "10" className={ styles.enterUser }
                onChange={ (event) => setSurName(event.target.value) }></input></div>
            
            <div>New Address: <br/><input type="text" placeholder={props.user.address} minLength = "20" className={ styles.enterUrl }
                onChange={ (event) => setAddress(event.target.value) }></input></div>
            
            <div>New Postnumber: <br/><input type="text" minLength = "11" placeholder={props.user.postNumber} className={ styles.enterUser }
